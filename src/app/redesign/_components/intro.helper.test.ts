...
await test("only fresh homepage loads and reloads play greetings", () => {
  assert.equal(shouldPlayIntro("/redesign", "navigate", false), true);
  assert.equal(shouldPlayIntro("/redesign", "reload", false), true);
  assert.equal(shouldPlayIntro("/redesign", "back_forward", false), false);
  assert.equal(shouldPlayIntro("/redesign/posts", "reload", false), false);
  assert.equal(shouldPlayIntro("/redesign", "navigate", true), false);
  assert.equal(shouldPlayIntro("/redesign", undefined, false), true);
});
